...
const StandupCallSession = () => {
  ...
  const CONTINUOUS_FACE_CHECK_INTERVAL = 10000; // 10 seconds
  const MAX_CONTINUOUS_FACE_WARNINGS = 2; // 2 warnings, terminate on 3rd
  ...
  // 1. ADD a ref to track if continuous verification is already running
  const isContinuousVerificationRunningRef = useRef(false);
  const isPerformingCheckRef = useRef(false);  // Mutex for individual checks
  ...
  isPerformingCheckRef.current = false;
    if (continuousFaceStreamRef.current) {
      continuousFaceStreamRef.current.getTracks().forEach(track => track.stop());
  ...
  const terminateSession = useCallback((reason) => {
    ...
    if (isSessionCompleteRef.current || isSessionComplete || sessionCompletionData) {
      console.log('⏭️ Skipping termination - session already completed normally');
      return;
    }
  ...
  useEffect(() => {
    if (standupCallAPI) {
        standupCallAPI.setSessionEndedCallback((endData) => {
            ...
            console.log('📍 Silence termination - redirecting to dashboard immediately');
            navigate('/student/daily-standups');
        });
    }
...
}, [navigate, stopContinuousFaceCamera]);
  ...
  if (standupCallAPI && standupCallAPI.onEvaluationComplete) {
    standupCallAPI.onEvaluationComplete((result) => {
      ...
      if (isSilenceTermination) {
        console.log('🔕 Session ended due to silence - skipping feedback, redirecting to dashboard');
...
}, [realSessionId, feedbackSubmitted, navigate, stopContinuousFaceCamera]);
  // ==================== TRIGGER REPORT GENERATION ON COMPLETE ====================
  useEffect(() => {
    if (sessionState === 'complete' && !showEvaluationReport && !isGeneratingReport) {
  ...
  useEffect(() => {
    if (sessionState === 'complete' && !showFeedbackForm && !feedbackSubmitted && !isGeneratingReport) {
      setShowFeedbackForm(true);
    }
  ...
  useEffect(() => {
    if (!showVideoInterface || sessionState === 'complete' || sessionState === 'error') {
      return;
    }
    ...
    const handleKeyDown = (e) => {
      ...
      if (e.key === 'Escape') {
        ...
        terminateSession('You pressed ESC key. Exiting the session is not allowed during the assessment.');
        return false;
      }
      ...
      if (e.key === 'F11') {
        ...
        terminateSession('You attempted to exit fullscreen mode. This is not allowed during the assessment.');
        return false;
      }
      ...
      if (e.altKey && e.key === 'Tab') {
        e.preventDefault();
        e.stopPropagation();
        terminateSession('You attempted to switch applications. This is not allowed.');
        return false;
      }
      ...
      if (e.altKey && e.key === 'F4') {
        e.preventDefault();
        e.stopPropagation();
        terminateSession('You attempted to close the window. This is not allowed.');
        return false;
      }
      ...
      if (e.ctrlKey && (e.key === 'w' || e.key === 'W')) {
        e.preventDefault();
        e.stopPropagation();
        terminateSession('You attempted to close the tab. This is not allowed.');
        return false;
      }
      ...
      if (e.ctrlKey && e.key === 'Tab') {
        e.preventDefault();
        e.stopPropagation();
        terminateSession('You attempted to switch browser tabs. This is not allowed.');
        return false;
      }
  ...
  const handleVisibilityChange = () => {
    ...
    if (document.hidden) {
      ...
      terminateSession('Tab or application switching detected. This is not allowed during the assessment.');
    }
  };
  ...
  const handleWindowBlur = () => {
    ...
    terminateSession('Window lost focus. You may have switched to another application. This is not allowed.');
  };
  
  const handleBeforeUnload = (e) => {
    e.preventDefault();
    e.returnValue = 'Your session will be terminated if you leave. Are you sure?';
  ...
  useEffect(() => {
    const handleFullscreenChange = () => {
      ...
      // ✅ NEW: Don't terminate if session already completed normally
      if (isSessionCompleteRef.current || isSessionComplete || sessionCompletionData) {
        console.log('✅ Fullscreen exited after session completion - allowing gracefully');
        return;
      }

      if (!isCurrentlyFullscreen && showVideoInterface && sessionState !== 'ready' && sessionState !== 'complete' && sessionState !== 'error') {
        console.log('🛑 Fullscreen exited - IMMEDIATE TERMINATION');
        terminateSession('You exited fullscreen mode. The assessment must remain in fullscreen.');
      }
    };
  ...
  useEffect(() => {
  // Only trigger when:
  // 1. Video interface is showing
  // 2. Face was verified
  // 3. Session is active (not complete or error)
  if (showVideoInterface && faceVerified && sessionState !== 'complete' && sessionState !== 'error') {
    console.log('🔐 Starting security features...');
    
    // Enter fullscreen mode immediately
    const enterFS = async () => {
  // Guard: Only try once AND check if already fullscreen
  ...
  hasEnteredFullscreenRef.current = true;  // Mark as attempted
  ...
  try {
    ...
    // Don't treat this as critical - user might have denied or browser blocked it
  }
};
    
    enterFS();
    
    // ✅ FIX: Single delayed start - removed duplicate setTimeout
    const faceVerificationTimer = setTimeout(() => {
      console.log('🔄 Starting continuous face verification...');
      startContinuousFaceVerification();
    }, 2000);
    
    return () => {
      clearTimeout(faceVerificationTimer);
    };
  }
  ...
  const startAudioEnhancedConversation = async () => {
    try {
      ...
      setAudioQuality(prev => ({ ...prev, isCalibrating: true, calibrationProgress: 0 }));
      ...
      audioProcessorRef.current.onCalibrationComplete = (calibrationData) => {
        ...
        setCurrentMessage('Native audio enhancement active! Connecting to AI backend...');
  ...
  const startBackendConnection = async () => {
    try {
      ...
      setCurrentMessage('Native audio enhancement active! Professional quality audio ready.');
 ...
 */
  ...
  try {
    ...
    if (result.verified) {
      ...
      switch (result.error_type) {
        case 'no_face': msg = '❌ No face detected.'; break;
        case 'multiple_faces': msg = '👥 Multiple faces detected.'; break;
        case 'face_turned': msg = '↩️ Look at the camera.'; break;
        case 'face_obstructed': msg = '🚫 Face obstructed.'; break;
        case 'poor_quality': msg = '💡 Improve lighting.'; break;
        case 'face_mismatch': msg = '🚨 Face mismatch.'; break;
        case 'face_too_small': msg = '📏 Move closer.'; break;
        case 'face_too_close': msg = '📏 Move back.'; break;
        default: break;
      }
  ...
  const handleVoiceWarning = (warningData) => {
    ...
    if (warningData.shouldTerminate || warningData.warningCount >= 3) {
      ...
      terminateSession('Voice verification failed: Your voice does not match the registered voice profile.');
    }
  };
 ...
 */
  ...
  if (isContinuousVerificationRunningRef.current) {
    console.log('⏭️ Continuous face verification already running, skipping duplicate start');
    return;
  }
  ...
  const handleFullscreenChange = () => {
    ...
    if (!isCurrentlyFullscreen && sessionState !== 'ready' && sessionState !== 'complete' && sessionState !== 'error' && showVideoInterface) {
      console.log('🛑 Fullscreen exited - IMMEDIATE TERMINATION');
      terminateSession('You exited fullscreen mode. This is not allowed during the assessment. The session must remain in fullscreen.');
    }
  };
   ...
   */
  const handleVisibilityChange = () => {
    if (document.hidden && sessionState !== 'ready' && sessionState !== 'complete' && sessionState !== 'error' && showVideoInterface) {
      console.log('🛑 Tab switch detected - IMMEDIATE TERMINATION');
      
      // Immediate termination - no warnings for tab switching
      terminateSession('Tab/window switching is not allowed during the session. This is a security violation.');
    }
  };
  ...
  const handleKeyDown = (e) => {
    ...
    if (e.altKey && e.key === 'Tab') {
      ...
      terminateSession('You attempted to switch applications. This is not allowed during the assessment.');
      return false;
    }
    ...
    if (e.altKey && e.key === 'F4') {
      ...
      terminateSession('You attempted to close the window. This is not allowed during the assessment.');
      return false;
    }
    ...
    if (e.ctrlKey && e.key === 'w') {
      ...
      terminateSession('You attempted to close the tab. This is not allowed during the assessment.');
      return false;
    }
    ...
    if (e.ctrlKey && e.key === 'Tab') {
      ...
      terminateSession('You attempted to switch browser tabs. This is not allowed during the assessment.');
      return false;
    }
    ...
    if (e.key === 'Meta' || e.key === 'OS') {
      ...
      terminateSession('You pressed the Windows key. This is not allowed during the assessment.');
      return false;
    }
  };
  ...
  const checkBiometricRequirement = async () => {
    try {
      ...
      console.error(
        'Error checking biometric requirement:',
        error
      );
  ...
  const handleDownloadPdf = (sessId) => {
    ...
    if (targetSessionId) {
      if (standupCallAPI && standupCallAPI.downloadPdfReport) {
        ...
        const pdfUrl = `https://192.168.48.201:8020/daily_standup/download_results/${targetSessionId}`;
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      ...
      if (standupCallAPI && standupCallAPI.submitFeedback) {
        ...
        const response = await fetch(`https://192.168.48.201:8020/daily_standup/submit_feedback`, {
          method: 'POST',
        ...
        if (!response.ok) {
          console.warn('Feedback submission returned non-OK status:', response.status);
        }
      }
  ...
  if (communication_score < 60) {
    if (communication_score < 40) {
      allWeaknesses.push("Poor communication clarity - responses were often unclear or incomplete");
    }
    if (raw_stats.irrelevant_count > 3) {
      allWeaknesses.push("Frequent off-topic responses indicate difficulty understanding questions");
    }
    if (raw_stats.repeat_requests_count > 2) {
      allWeaknesses.push("Multiple requests for question repetition may indicate attention or comprehension issues");
    }
...
if (communication_score < 60) {
  if (communication_score < 40) {
    allRecommendations.push("Practice articulating technical concepts clearly and concisely");
    allRecommendations.push("Work on structuring responses with clear beginning, middle, and end");
  }
  if (raw_stats.irrelevant_count > 3) {
    allRecommendations.push("Focus on understanding the question fully before responding");
    allRecommendations.push("Practice active listening during technical discussions");
  }
  if (raw_stats.repeat_requests_count > 2) {
    allRecommendations.push("Improve focus and attention during conversations");
    allRecommendations.push("Take notes while listening to complex questions");
  }
  if (raw_stats.silent_count > 3) {
    allRecommendations.push("Practice thinking out loud - share your thought process even if unsure");
    allRecommendations.push("Don't hesitate to ask clarifying questions instead of staying silent");
  }
}
// ========== END COMMUNICATION RECOMMENDATIONS LOGIC ==========
      ...
      <Box sx={{ minHeight: '100vh', backgroundColor: '#f0f4f8', pb: 6 }}>
          ...
          <Container maxWidth="lg">
            <Box display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" gap={2}>
              <Box display="flex" alignItems="center" gap={2}>
                <IconButton onClick={handleBackFromReport} sx={{ color: '#ffffff' }}>
                  <ArrowBack />
                </IconButton>
                <Box>
                  ...
                  <Typography variant="body1" sx={{ opacity: 0.9 }}>
                    {session_info.student_name || 'Student'} • {new Date().toLocaleDateString()}
                  </Typography>
                </Box>
              </Box>
        ...
        <Container maxWidth="lg" sx={{ mt: 4 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', mb: 4 }}>
            ...
            <Typography variant="h3" sx={{ color: scoreColor, fontWeight: 'bold', textAlign: 'center', mt: 2 }}>
              {overall_score}/100
            </Typography>
          ...
          <Box sx={{ textAlign: 'center' }}>
            <Typography variant="h5" gutterBottom sx={{ fontWeight: 'bold', color: '#0f172a' }}>
              Performance Summary
            </Typography>
            <Typography variant="body1" sx={{ color: '#475569', lineHeight: 1.8, maxWidth: '800px', mx: 'auto' }}>
              {summary}
            </Typography>
          </Box>
        </ScoreCard>
            ...
            <Grid item xs={12} md={4}>
              <Card sx={{ height: '100%' }}>
                <CardContent>
                  ...
                  <Typography variant="h4" sx={{ color: getScoreColor(technical_score), fontWeight: 'bold' }}>
                    {technical_score}/100
                  </Typography>
                  <LinearProgress 
                    variant="determinate" 
                    ...
                    sx={{ 
                      ...
                      '& .MuiLinearProgress-bar': { bgcolor: getScoreColor(technical_score) }
                    }} 
                  />
                </CardContent>
              </Card>
            ...
            <Grid item xs={12} md={4}>
              <Card sx={{ height: '100%' }}>
                <CardContent>
                  ...
                  <Typography variant="h4" sx={{ color: getScoreColor(communication_score), fontWeight: 'bold' }}>
                  ...
                  <LinearProgress 
                    ...
                    sx={{ 
                      ...
                      '& .MuiLinearProgress-bar': { bgcolor: getScoreColor(communication_score) }
                    }} 
                  />
                </CardContent>
              </Card>
            ...
            <Grid item xs={12} md={4}>
              <Card sx={{ height: '100%' }}>
                <CardContent>
                  ...
                  <Typography variant="h4" sx={{ color: getScoreColor(attentiveness_score), fontWeight: 'bold' }}>
                  ...
                  <LinearProgress 
                    ...
                    sx={{ 
                      ...
                      '& .MuiLinearProgress-bar': { bgcolor: getScoreColor(attentiveness_score) }
                    }} 
                  />
                </CardContent>
              </Card>
              ...
              <Grid container spacing={2} sx={{ mt: 1 }}>
                <Grid item xs={6} sm={4} md={2}>
                  <Box textAlign="center">
                    <Typography variant="h4" color="primary">{raw_stats.total_questions || 0}</Typography>
                    <Typography variant="body2" color="text.secondary">Total Questions</Typography>
                  </Box>
                </Grid>
                <Grid item xs={6} sm={4} md={2}>
                  <Box textAlign="center">
                    <Typography variant="h4" color="success.main">{raw_stats.answered_count || 0}</Typography>
                    <Typography variant="body2" color="text.secondary">Answered</Typography>
                  </Box>
                </Grid>
                <Grid item xs={6} sm={4} md={2}>
                  <Box textAlign="center">
                    <Typography variant="h4" color="warning.main">{raw_stats.skipped_count || 0}</Typography>
                    <Typography variant="body2" color="text.secondary">Skipped</Typography>
                  </Box>
                </Grid>
                <Grid item xs={6} sm={4} md={2}>
                  <Box textAlign="center">
                    <Typography variant="h4" color="grey.500">{raw_stats.silent_count || 0}</Typography>
                    <Typography variant="body2" color="text.secondary">Silent</Typography>
                  </Box>
                </Grid>
                <Grid item xs={6} sm={4} md={2}>
                  <Box textAlign="center">
                    <Typography variant="h4" color="error.main">{raw_stats.irrelevant_count || 0}</Typography>
                    <Typography variant="body2" color="text.secondary">Off-Topic</Typography>
                  </Box>
                </Grid>
                <Grid item xs={6} sm={4} md={2}>
                  <Box textAlign="center">
                    <Typography variant="h4" color="info.main">{raw_stats.duration_minutes?.toFixed(1) || 0}</Typography>
                    ...
                    <List dense>
                      {allWeaknesses.map((weakness, index) => (
                        <ListItem key={index}>
                          ...
                          <ListItemText primary={typeof weakness === 'object' ? weakness.area : weakness} />
                        </ListItem>
                      ))}
                    </List>
                  ) : (
                    ...
                    <List dense>
                      {strengths.map((strength, index) => (
                        <ListItem key={index}>
                          ...
                          <ListItemText primary={typeof strength === 'object' ? strength.area : strength} />
                        </ListItem>
                      ))}
                    </List>
                  ) : (
          ...
          <Alert severity="info" sx={{ mt: 3 }}>
            <Typography variant="body2">
              📄 Download the PDF report to view detailed question-by-question analysis
            </Typography>
          </Alert>
          ...
          {/* Recommendations */}
          <Card sx={{ mt: 3, borderTop: `4px solid ${theme.palette.info.main}` }}>
            <CardContent>
              ...
              {allRecommendations.length > 0 ? (
                <List>
                  {recommendations.map((rec, index) => (
                    <ListItem key={index}>
                      <ListItemIcon>
                        <Avatar sx={{ bgcolor: 'info.main', width: 28, height: 28, fontSize: '0.875rem' }}>
                          {index + 1}
                        </Avatar>
                      </ListItemIcon>
  ...
  const renderGeneratingReport = () => (
    ...
    >
      ...
      <Typography variant="h5" sx={{ mt: 4, fontWeight: 'bold', color: '#0f172a' }}>
      ...
      <Box sx={{ mt: 4, display: 'flex', gap: 2, flexWrap: 'wrap', justifyContent: 'center' }}>
        <Chip icon={<CheckCircle />} label="Analyzing answers" color="primary" />
        <Chip icon={<Psychology />} label="Calculating scores" color="primary" />
        <Chip icon={<Lightbulb />} label="Generating feedback" color="primary" />
      </Box>
    </Box>
  );
      ...
      <Box display="flex" alignItems="center" gap={2}>
        ...
        <Box>
          ...
          <Typography variant="body2" sx={{ opacity: 0.9 }}>
            Please read carefully before starting - Violations will terminate your session
          </Typography>
        </Box>
      </Box>
    </DialogTitle>
    ...
    <DialogContent sx={{ py: 4, px: 4 }}>
      ...
      <Alert severity="error" sx={{ mb: 3, borderRadius: 2 }}>
        <Typography variant="body1" sx={{ fontWeight: 'bold' }}>
          🔒 SECURITY NOTICE: This session uses biometric authentication (Face & Voice verification). 
          Any security violation will result in <strong>IMMEDIATE SESSION TERMINATION</strong>.
        </Typography>
      </Alert>

      {/* ==================== SECURITY REQUIREMENTS SECTION ==================== */}
      <Box sx={{ mb: 4, p: 3, bgcolor: alpha(theme.palette.error.main, 0.05), borderRadius: 2, border: `2px solid ${alpha(theme.palette.error.main, 0.3)}` }}>
        <Typography variant="h6" sx={{ fontWeight: 'bold', color: 'error.main', mb: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
        ...
        <Box sx={{ mb: 3 }}>
          ...
          <Typography variant="body2" color="text.secondary" component="div">
            <ul style={{ margin: '8px 0', paddingLeft: 20 }}>
              <li><strong>Plain Background Required:</strong> Sit in front of a plain, uncluttered background (wall preferred)</li>
              <li><strong>Only YOU in Frame:</strong> No other people should be visible in the camera. Multiple faces = Session Terminated</li>
              <li><strong>No Prohibited Objects:</strong> Keep the following items OUT of camera view:
              ...
              <li><strong>Face Always Visible:</strong> Look at the camera throughout the session. Don't look down, away, or cover your face</li>
              <li><strong>Good Lighting:</strong> Ensure your face is well-lit and clearly visible</li>
              <li><strong>3 Warnings = Termination:</strong> After 3 face verification failures, session ends immediately</li>
            </ul>
          </Typography>
        </Box>
        ...
        <Box sx={{ mb: 3 }}>
          ...
          <Typography variant="body2" color="text.secondary" component="div">
            <ul style={{ margin: '8px 0', paddingLeft: 20 }}>
              <li><strong>Your Voice Only:</strong> Only the registered student's voice should respond</li>
              <li><strong>No Help from Others:</strong> Do not let anyone else answer questions for you</li>
              <li><strong>Voice Mismatch Detection:</strong> If your voice doesn't match the registered voice profile, you'll receive a warning</li>
              <li><strong>3 Warnings = Termination:</strong> After 3 voice verification failures, session ends immediately</li>
            </ul>
          </Typography>
        </Box>
        ...
        <Box>
          ...
          <Typography variant="body2" color="text.secondary" component="div">
            <ul style={{ margin: '8px 0', paddingLeft: 20 }}>
              <li><strong>No Tab Switching:</strong> Switching browser tabs will terminate the session immediately</li>
              <li><strong>No Window Switching:</strong> Alt+Tab or clicking other applications will terminate the session</li>
              <li><strong>No Exiting Fullscreen:</strong> Pressing ESC or F11 will terminate the session</li>
              <li><strong>No Keyboard Shortcuts:</strong> Ctrl+Tab, Ctrl+W, Alt+F4 are all blocked</li>
              <li><strong>Stay in Fullscreen:</strong> The session runs in fullscreen mode - do not attempt to exit</li>
            </ul>
          </Typography>
        </Box>
      </Box>
      ...
      <Alert severity="warning" sx={{ mb: 3, borderRadius: 2 }}>
        <Typography variant="body2">
          <strong>⚠️ Warning System:</strong> You get a maximum of <strong>3 warnings</strong> for each security check 
          (face and voice). On the 3rd violation, your session will be <strong>automatically terminated</strong> and 
          logged for review.
        </Typography>
      </Alert>
      ...
      <List>
        ...
        <ListItem sx={{ py: 2, px: 0 }}>
          <ListItemIcon>
            <Avatar sx={{ bgcolor: theme.palette.primary.main, width: 36, height: 36 }}>
              <Typography variant="body1" sx={{ fontWeight: 'bold', color: '#fff' }}>1</Typography>
            </Avatar>
          </ListItemIcon>
          <ListItemText
            primary={
            ...
            secondary={
              <Typography variant="body2" color="text.secondary">
                Always wait to finish asking the question completely before you start speaking. 
                Interrupting may cause your response to be missed.
              </Typography>
            }
          />
        </ListItem>
        ...
        <ListItem sx={{ py: 2, px: 0 }}>
          <ListItemIcon>
            <Avatar sx={{ bgcolor: theme.palette.error.main, width: 36, height: 36 }}>
              <Typography variant="body1" sx={{ fontWeight: 'bold', color: '#fff' }}>2</Typography>
            </Avatar>
          </ListItemIcon>
          <ListItemText
            primary={
            ...
            secondary={
              <Typography variant="body2" color="text.secondary">
                If you remain silent (no response) for <strong>extended periods repeatedly</strong>, your session may be 
                <strong> automatically terminated</strong>. Always respond, even if you don't know the answer.
              </Typography>
            }
          />
        </ListItem>
        ...
        <ListItem sx={{ py: 2, px: 0 }}>
          <ListItemIcon>
            <Avatar sx={{ bgcolor: theme.palette.warning.main, width: 36, height: 36 }}>
              <Typography variant="body1" sx={{ fontWeight: 'bold', color: '#fff' }}>3</Typography>
            </Avatar>
          </ListItemIcon>
          <ListItemText
            primary={
            ...
            secondary={
              <Typography variant="body2" color="text.secondary">
                It's okay to say <strong>"I don't know"</strong> or <strong>"I'm not sure"</strong> or <strong>"Skip this question"</strong>. 
                This is better than staying silent. This will move to the next question.
              </Typography>
            }
          />
        </ListItem>
        ...
        <ListItem sx={{ py: 2, px: 0 }}>
          <ListItemIcon>
            <Avatar sx={{ bgcolor: theme.palette.info.main, width: 36, height: 36 }}>
              <Typography variant="body1" sx={{ fontWeight: 'bold', color: '#fff' }}>4</Typography>
            </Avatar>
          </ListItemIcon>
          <ListItemText
            primary={
            ...
            secondary={
              <Typography variant="body2" color="text.secondary">
                If you didn't hear the question clearly, say <strong>"Please repeat the question"</strong> or 
                <strong>"Can you say that again?"</strong>. The AI will repeat the question for you.
              </Typography>
            }
          />
        </ListItem>
        ...
        <ListItem sx={{ py: 2, px: 0 }}>
          <ListItemIcon>
            <Avatar sx={{ bgcolor: theme.palette.success.main, width: 36, height: 36 }}>
              <Typography variant="body1" sx={{ fontWeight: 'bold', color: '#fff' }}>5</Typography>
            </Avatar>
          </ListItemIcon>
          <ListItemText
            primary={
            ...
            secondary={
              <Typography variant="body2" color="text.secondary">
                Answer questions related to the topic being discussed. <strong>Off-topic or irrelevant answers</strong> will 
        ...
        <ListItem sx={{ py: 2, px: 0 }}>
          <ListItemIcon>
            <Avatar sx={{ bgcolor: theme.palette.secondary.main, width: 36, height: 36 }}>
              <Typography variant="body1" sx={{ fontWeight: 'bold', color: '#fff' }}>6</Typography>
            </Avatar>
          </ListItemIcon>
          <ListItemText
            primary={
            ...
            secondary={
              <Typography variant="body2" color="text.secondary">
                Each standup session lasts approximately <strong>15 minutes</strong>. Make sure you have a quiet 
        ...
        <ListItem sx={{ py: 2, px: 0 }}>
          <ListItemIcon>
            <Avatar sx={{ bgcolor: '#6b7280', width: 36, height: 36 }}>
              <Typography variant="body1" sx={{ fontWeight: 'bold', color: '#fff' }}>7</Typography>
            </Avatar>
          </ListItemIcon>
          <ListItemText
            primary={
            ...
            secondary={
              <Typography variant="body2" color="text.secondary">
                Speak clearly and at a normal pace. The system uses speech recognition, so <strong>mumbling or 
                speaking too fast</strong> may result in transcription errors.
              </Typography>
            }
          />
        </ListItem>
        ...
        <ListItem sx={{ py: 2, px: 0 }}>
          <ListItemIcon>
            <Avatar sx={{ bgcolor: theme.palette.info.main, width: 36, height: 36 }}>
              <Typography variant="body1" sx={{ fontWeight: 'bold', color: '#fff' }}>8</Typography>
            </Avatar>
          </ListItemIcon>
          <ListItemText
            primary={
            ...
            secondary={
              <Typography variant="body2" color="text.secondary">
                After completing the session, you can <strong>download a PDF report</strong> with detailed question-by-question analysis.
              </Typography>
            }
          />
        </ListItem>
        ...
        <ListItem sx={{ py: 2, px: 0 }}>
          <ListItemIcon>
            <Avatar sx={{ bgcolor: theme.palette.success.main, width: 36, height: 36 }}>
              <Typography variant="body1" sx={{ fontWeight: 'bold', color: '#fff' }}>9</Typography>
            </Avatar>
          </ListItemIcon>
          <ListItemText
            primary={
            ...
            secondary={
              <Typography variant="body2" color="text.secondary">
                After completion, you will be asked for feedback to help us improve the experience.
              </Typography>
            }
          />
        </ListItem>
      ...
      >
        <Typography variant="subtitle1" sx={{ fontWeight: 'bold', color: theme.palette.success.main, mb: 1 }}>
          ✅ Recommended Setup Before Starting
        </Typography>
        <Typography variant="body2" color="text.secondary">
          • <strong>Plain background:</strong> Sit facing a plain wall with no distractions<br />
          • <strong>Good lighting:</strong> Face a light source so your face is clearly visible<br />
          • <strong>Clear desk:</strong> Remove phones, books, and other materials from camera view<br />
          • <strong>Quiet environment:</strong> Find a quiet place with minimal background noise<br />
          • <strong>Stable internet:</strong> Ensure you have a reliable internet connection<br />
          • <strong>Headset/Earphones:</strong> Use for better audio quality (recommended)<br />
          • <strong>Close other apps:</strong> Close all unnecessary browser tabs and applications<br />
          • <strong>Be alone:</strong> Make sure no one else is in the frame during the session
        </Typography>
      </Box>
      ...
      >
        <Typography variant="subtitle1" sx={{ fontWeight: 'bold', color: theme.palette.error.main, mb: 1 }}>
          ⚠️ Final Reminder
        </Typography>
        <Typography variant="body2" color="text.secondary">
          Once you start the session, it will run in <strong>fullscreen mode</strong>. Any attempt to exit fullscreen, 
          switch tabs, or use prohibited keyboard shortcuts will <strong>immediately terminate</strong> your session. 
          Make sure you are ready and have at least 15 minutes of uninterrupted time.
        </Typography>
      </Box>
    </DialogContent>

    {/* Dialog Actions */}
    <DialogActions sx={{ px: 4, py: 3, bgcolor: alpha(theme.palette.grey[100], 0.5) }}>
      <Button
        variant="outlined"
      ...
      >
        ...
        <DialogContent sx={{ py: 4, px: 4 }}>
            ...
            <Grid item xs={12} sm={6}>
              <Box>
                ...
                <Rating
                  ...
                  onChange={(e, value) => handleFeedbackChange('overallExperience', value)}
                  size="large"
            ...
            <Grid item xs={12} sm={6}>
              <Box>
                ...
                <Rating
                  ...
                  onChange={(e, value) => handleFeedbackChange('audioQuality', value)}
                  size="large"
            ...
            <Grid item xs={12} sm={6}>
              <Box>
                ...
                <Rating
                  ...
                  onChange={(e, value) => handleFeedbackChange('questionClarity', value)}
                  size="large"
            ...
            <Grid item xs={12} sm={6}>
              <Box>
                ...
                <Rating
                  ...
                  onChange={(e, value) => handleFeedbackChange('systemResponsiveness', value)}
                  size="large"
          ...
          <Box sx={{ mb: 3 }}>
            <FormControl component="fieldset">
              ...
              <RadioGroup
                ...
                onChange={(e) => handleFeedbackChange('difficultyLevel', e.target.value)}
              >
                <FormControlLabel value="too_easy" control={<Radio />} label="Too Easy" />
                <FormControlLabel value="easy" control={<Radio />} label="Easy" />
                <FormControlLabel value="moderate" control={<Radio />} label="Moderate" />
                <FormControlLabel value="challenging" control={<Radio />} label="Challenging" />
                <FormControlLabel value="too_difficult" control={<Radio />} label="Too Difficult" />
              </RadioGroup>
            </FormControl>
          </Box>
          ...
          <Box sx={{ mb: 3 }}>
            <FormControl component="fieldset">
              ...
              <RadioGroup
                ...
                onChange={(e) => handleFeedbackChange('wouldRecommend', e.target.value)}
              >
                <FormControlLabel value="yes" control={<Radio color="success" />} label="Yes, definitely" />
                <FormControlLabel value="maybe" control={<Radio color="warning" />} label="Maybe" />
          ...
          <TextField
            ...
            onChange={(e) => handleFeedbackChange('otherIssues', e.target.value)}
            variant="outlined"
            sx={{ mb: 3 }}
          />
          ...
          >
            ...
            <Typography variant="body2" color="text.secondary">
              Visit <strong>iMentora</strong> for personalized mentorship, additional learning resources, 
              and to connect with experienced professionals who can help you improve your technical skills 
        ...
        <DialogActions sx={{ px: 4, py: 3, bgcolor: alpha(theme.palette.grey[100], 0.5) }}>
          ...
          <Button
            ...
            disabled={isSubmittingFeedback}
            startIcon={isSubmittingFeedback ? <CircularProgress size={20} color="inherit" /> : <CheckCircle />}
            sx={{
              px: 4,
              py: 1,
              background: 'linear-gradient(135deg, #1a5276 0%, #0d9488 100%)',
 ...
 const renderFaceVerification = () => (
  ...
  const renderVoiceWarning = () => (
    <Fade in={showVoiceWarning}>
      <Alert
        severity={
          voiceWarnings >= MAX_VOICE_WARNINGS - 1 ? 'error' : 'warning'
        }
        sx={{
          position: 'fixed',
          top: 100,
          ...
          {[...Array(MAX_VOICE_WARNINGS)].map((_, i) => (
            <Box
              ...
              sx={{
                ...
                borderRadius: '50%',
                bgcolor:
                  i < voiceWarnings ? 'error.main' : 'grey.300'
              }}
            />
          ))}
  ...
  <Fade in={true}>
    <Box sx={{ width: '100%', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <Box sx={{ minHeight: '100vh', backgroundColor: '#f0f4f8' }}>
                  ...
                  <Box display="flex" justifyContent="space-between" alignItems="center">
                    <Box display="flex" alignItems="center" gap={2}>
                      ...
                      <Box>
                        ...
                        <Typography variant="body1" sx={{ opacity: 0.9 }}>
                          ...
                          {isAudioEnhanced && ' • Native Audio Enhancement: Active'}
                        </Typography>
                      </Box>
                    </Box>
                    ...
                    <Box display="flex" gap={1} alignItems="center">
                      {showVideoInterface && (
                        <>
                          <Chip
                            label={`Face: ${continuousFaceWarnings}/${MAX_CONTINUOUS_FACE_WARNINGS + 1}`}
                            size="small"
                            sx={{
                              bgcolor: continuousFaceWarnings > 0 ? alpha('#f44336', 0.3) : alpha('#ffffff', 0.2),
                              color: '#ffffff'
                            }}
                          />
                          <Chip
                            ...
                            size="small"
                            sx={{
                              bgcolor: fullscreenExitWarnings > 0 ? alpha('#f44336', 0.3) : alpha('#ffffff', 0.2),
                              color: '#ffffff'
                            }}
                          />
                          <Chip
                            ...
                            size="small"
                            sx={{
                              bgcolor: tabSwitchWarnings > 0 ? alpha('#f44336', 0.3) : alpha('#ffffff', 0.2),
                              color: '#ffffff'
                            }}
                          />
                        </>
              ...
              <Container maxWidth="xl" sx={{ py: 4 }}>
                ...
                {audioQuality.isCalibrating && (
                  <Card sx={{ mb: 4, p: 3, bgcolor: alpha(theme.palette.info.main, 0.05) }}>
                    <Typography variant="h6" color="info.main" gutterBottom>
                      🎯 Native Browser Audio Enhancement Calibration
                    </Typography>
                    <Typography variant="body1" sx={{ mb: 2 }}>
                      Initializing browser's built-in noise suppression and echo cancellation...
                    </Typography>
                    <LinearProgress
                      variant="determinate"
                    ...
                    />
                    <Typography variant="body2" color="text.secondary">
                      Calibration Progress: {Math.round(audioQuality.calibrationProgress)}%
                    </Typography>
                  </Card>
                )}
                ...
                {isAudioEnhanced && !audioQuality.isCalibrating && (
                  <Card sx={{ mb: 4, p: 3, bgcolor: alpha(theme.palette.success.main, 0.05) }}>
                    <Typography variant="h6" color="success.main" gutterBottom>
                      🎤 Native Browser Audio Enhancement Active
                    </Typography>
                    <Typography variant="body2" sx={{ mb: 3, fontStyle: 'italic' }}>
                      Using the same noise cancellation technology as Google Meet, Zoom, and Discord
                    </Typography>
                    <Grid container spacing={3}>
                      <Grid item xs={12} md={4}>
                        ...
                        />
                        <Typography variant="body2" color="text.secondary">
                          {audioQuality.gateActive ? '🗣️ Voice Detected' : '🔇 No Voice'}
                        </Typography>
                      </Grid>
                      <Grid item xs={12} md={4}>
                        <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
                        ...
                        />
                        <Typography variant="body2" color="text.secondary">
                          {Math.round(Math.max(0, 100 - audioQuality.noiseLevel))}% Noise Filtered
                        </Typography>
                      </Grid>
                      <Grid item xs={12} md={4}>
                        <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
                        ...
                        />
                        <Typography variant="body2" color="text.secondary">
                          {Math.round(audioQuality.qualityScore)}% Professional Quality
                        </Typography>
                      </Grid>
                    </Grid>
                  </Card>
                ...
                {showVideoInterface && (
                  ...
                  >
                    ...
                    <Box sx={{ position: 'absolute', top: 10, left: '50%', transform: 'translateX(-50%)', zIndex: 10001, width: '90%', maxWidth: 500 }}>
                      {showContinuousFaceWarning && (
                        <Alert 
                          severity={continuousFaceWarnings >= 2 ? "error" : "warning"} 
                          sx={{ mb: 1, borderRadius: 2, boxShadow: 8 }}
                        >
                          <Typography variant="subtitle2" sx={{ fontWeight: 'bold' }}>
                            ⚠️ Security Warning ({continuousFaceWarnings}/3)
                          </Typography>
                          <Typography variant="body2">
                            {continuousFaceWarningMessage || 'Face verification issue detected'}
                          </Typography>
                          {continuousFaceWarnings >= 2 && (
                            <Typography variant="caption" color="error" sx={{ fontWeight: 'bold', display: 'block', mt: 0.5 }}>
                      ...
                      {showVoiceWarning && (
                        ...
                        >
                          ...
                          <Typography variant="body2">
                            {voiceWarningMessage || 'Voice verification failed'}
                          </Typography>
                          {voiceWarnings >= 2 && (
                            <Typography variant="caption" color="error" sx={{ fontWeight: 'bold', display: 'block', mt: 0.5 }}>
                              Next violation will terminate the session!
                    ...
                    <Box sx={{ 
                      ...
                      <Box display="flex" gap={1}>
                        <Chip
                          ...
                          size="small"
                          sx={{
                            bgcolor: continuousFaceWarnings > 0 ? 'error.main' : 'success.main',
                            color: '#fff'
                          }}
                        />
                        <Chip
                          ...
                          size="small"
                          sx={{
                            bgcolor: voiceWarnings > 0 ? 'error.main' : 'success.main',
                            color: '#fff'
                          }}
                        />
                      </Box>
                    ...
                    <Box sx={{ flex: 1, display: 'flex', p: 2, gap: 2 }}>
                      ...
                      <Box sx={{ 
                        ...
                        borderRadius: 3, 
                        overflow: 'hidden',
                        background: 'linear-gradient(135deg, #1a5276 0%, #1a73e8 100%)',
                        display: 'flex',
                        flexDirection: 'column',
                        alignItems: 'center',
                        justifyContent: 'center',
                        ...
                        <Chip
                          label={sessionState === 'speaking' ? 'Speaking...' : sessionState === 'listening' ? 'Listening...' : 'Ready'}
                          sx={{
                            bgcolor: sessionState === 'speaking' ? 'success.main' : 'rgba(255,255,255,0.2)',
                            color: '#fff'
                          }}
                        />
                      </Box>
                      ...
                      <Box sx={{ 
                        ...
                        {/* Continuous verification indicator */}
                        {isVerifyingContinuousFace && (
                          <Box sx={{
                            position: 'absolute',
                            top: 16,
                            right: 16,
                        ...
                        {cameraOn ? (
                          <>
                            ...
                            {sessionState === 'listening' && audioQuality.gateActive && (
                              <Box sx={{
                                position: 'absolute',
                                bottom: 60,
                                left: '50%',
                          ...
                          <Box sx={{
                            ...
                            <Typography variant="h5" sx={{ mb: 3 }}>Camera Off</Typography>
                            <Button
                              variant="contained"
                              startIcon={<Videocam />}
                    ...
                    {/* Hidden elements for continuous face verification */}
                    <video
                      ref={continuousFaceVideoRef}
                      autoPlay
                      playsInline
                      muted
                      style={{ display: 'none' }}
                    />
                    <canvas ref={continuousFaceCanvasRef} style={{ display: 'none' }} />
                  </Box>
                )}

                {/* ==================== MAIN INTERFACE CARD ==================== */}
                <StatusCard isActive={sessionState !== 'ready' && sessionState !== 'error'} elevation={8}>
                  <CardContent sx={{ p: showVideoInterface ? 4 : 6, textAlign: 'center' }}>
                    {!showVideoInterface && (
                    ...
                    {currentMessage && sessionState !== 'ready' && (
                      <Box sx={{ mb: showVideoInterface ? 3 : 4, maxWidth: 600, mx: 'auto' }}>
                        <Typography
                ...
                <Card sx={{
                  ...
                  <Typography variant="body1" sx={{ mb: 3, fontStyle: 'italic' }}>
                    Powered by the same WebRTC technology used in Google Meet, Zoom, Discord, and other professional applications
                  </Typography>
                  <Grid container spacing={3}>
                    <Grid item xs={12} sm={6} md={4}>
                      <Box sx={{ textAlign: 'center', p: 2 }}>
                        ...
                        <Typography variant="body2" color="text.secondary">
                          Browser's built-in AI-powered noise reduction removes background noise
                        </Typography>
                      </Box>
                    </Grid>
                    ...
                    <Grid item xs={12} sm={6} md={4}>
                      <Box sx={{ textAlign: 'center', p: 2 }}>
                        ...
                        <Typography variant="body2" color="text.secondary">
                          Eliminates echo and feedback for crystal clear audio quality
                        </Typography>
                      </Box>
                    </Grid>
                    ...
                    <Grid item xs={12} sm={6} md={4}>
                      <Box sx={{ textAlign: 'center', p: 2 }}>
                        ...
                        <Typography variant="body2" color="text.secondary">
                          Automatically adjusts volume levels for consistent audio output
                        </Typography>
                      </Box>
                    </Grid>
                    ...
                    <Grid item xs={12} sm={6} md={4}>
                      <Box sx={{ textAlign: 'center', p: 2 }}>
                        ...
                        <Typography variant="body2" color="text.secondary">
                          No processing delays - hardware-accelerated audio enhancement
                        </Typography>
                      </Box>
                    </Grid>
                    ...
                    <Grid item xs={12} sm={6} md={4}>
                      <Box sx={{ textAlign: 'center', p: 2 }}>
                        ...
                        <Typography variant="body2" color="text.secondary">
                          Same technology used in enterprise video conferencing solutions
                        </Typography>
                      </Box>
                    </Grid>
                ...
                <Card sx={{
                  ...
                  <Typography variant="h5" color="info.main" gutterBottom sx={{ fontWeight: 'bold' }}>
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={12} md={6}>
                      ...
                      <Typography variant="body2" sx={{ mb: 1 }}>
                        <strong>Echo Cancellation:</strong> true (hardware accelerated)
                      </Typography>
                      <Typography variant="body2" sx={{ mb: 1 }}>
                        <strong>Auto Gain Control:</strong> true (automatic leveling)
                      </Typography>
                      <Typography variant="body2" sx={{ mb: 1 }}>
                  ...
                  <Grid container spacing={3}>
                    <Grid item xs={12} md={6}>
                      ...
                      <Typography variant="body2" sx={{ mb: 2 }}>
                        • No custom JavaScript processing that can fail<br/>
                        • Same technology as Google Meet and Zoom<br/>
                        • Hardware-accelerated when available<br/>
                        • Tested by millions of users daily
                      </Typography>
                    </Grid>
                    <Grid item xs={12} md={6}>
                      <Typography variant="h6" gutterBottom sx={{ fontWeight: 'bold' }}>
                      ...
                      <Typography variant="body2" sx={{ mb: 2 }}>
                        • Zero added latency from processing<br/>
                        • No audio artifacts from JavaScript manipulation<br/>
                        • Optimized by browser vendors<br/>
                ...
                {process.env.NODE_ENV === 'development' && (
                  <Card sx={{
                    ...
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={12} sm={6} md={4}>
                        <Typography variant="body2" sx={{ fontFamily: 'monospace' }}>
                          <strong>Native Enhancement:</strong> {isAudioEnhanced.toString()}
                        </Typography>
                      </Grid>
                      <Grid item xs={12} sm={6} md={4}>
                        <Typography variant="body2" sx={{ fontFamily: 'monospace' }}>
                          <strong>Quality Score:</strong> {Math.round(audioQuality.qualityScore)}%
                        </Typography>
                      </Grid>
                      <Grid item xs={12} sm={6} md={4}>
                        <Typography variant="body2" sx={{ fontFamily: 'monospace' }}>
                          <strong>Voice Level:</strong> {Math.round(audioQuality.voiceLevel)}%
                        </Typography>
                      </Grid>
                      <Grid item xs={12} sm={6} md={4}>
                        <Typography variant="body2" sx={{ fontFamily: 'monospace' }}>
                          <strong>Noise Level:</strong> {Math.round(audioQuality.noiseLevel)}%
                        </Typography>
                      </Grid>
                      <Grid item xs={12} sm={6} md={4}>
                        <Typography variant="body2" sx={{ fontFamily: 'monospace' }}>
                      ...
                      <Grid item xs={12} sm={6} md={4}>
                        <Typography variant="body2" sx={{ fontFamily: 'monospace' }}>
                          <strong>Video Interface:</strong> {showVideoInterface.toString()}
                        </Typography>
                      </Grid>
                      <Grid item xs={12} sm={6} md={4}>
                        <Typography variant="body2" sx={{ fontFamily: 'monospace' }}>
                      ...
                      <Grid item xs={12} sm={6} md={4}>
                        <Typography variant="body2" sx={{ fontFamily: 'monospace' }}>
                          <strong>Face Verified:</strong> {faceVerified.toString()}
                        </Typography>
                      </Grid>
                    </Grid>
                  </Card>
      ...
      >
        <DialogTitle component="div" sx={{ color: 'error.main', textAlign: 'center' }}>
          <ErrorIcon sx={{ fontSize: 64, color: 'error.main', mb: 2 }} />
          <Typography variant="h5" component="span" sx={{ fontWeight: 'bold', display: 'block' }}>
            Session Terminated
          </Typography>
        </DialogTitle>
        <DialogContent>
