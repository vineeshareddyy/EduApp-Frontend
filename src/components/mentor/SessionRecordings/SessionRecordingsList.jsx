...
const MentorSessionRecordingsList= () => {
  ...
  const handleView = (recordingId) => {
    navigate(`/mentor/session-recordings/view/${recordingId}`);
  };
...
const handleEditSubmit = async () => {
  ...
  const handleAddNew = () => {
    navigate('/mentor/session-recordings/add');
  };
  ...
  const handlePlay = async (recording) => {
    try {
      ...
      const videoUrl = `http://192.168.48.31:8000/api/trainer/video/stream-direct/${recording.id}`;
...
export default MentorSessionRecordingsList;